...
public class CustomerOrderTests
{
    ...
    {
        var order = new CustomerOrder(Guid.NewGuid(), Guid.NewGuid());
        
        order.AddItem(new OrderItem(Guid.NewGuid(), "Item A", new Money(10m, "USD"), 2));
        order.AddItem(new OrderItem(Guid.NewGuid(), "Item B", new Money(5m, "USD"), 3));
    ...
    {
        var order = new CustomerOrder(Guid.NewGuid(), Guid.NewGuid());
        order.AddItem(new OrderItem(Guid.NewGuid(), "Item A", new Money(5m, "USD"), 1));
        Assert.Throws<InvalidOperationException>(() => 
            order.AddItem(new OrderItem(Guid.NewGuid(), "Item B", new Money(10m, "EUR"), 1)));
    }

    [Fact]
    public void LineTotal_rounds_away_from_zero()
    {
        var price = new Money(0.335m, "USD");
    ...
    public void AddItem_throws_after_confirm()
    {
        ...
        Assert.Throws<InvalidOperationException>(() => order.AddItem(new OrderItem(Guid.NewGuid(), "Item B", new Money(1m, "USD"), 1)));
    }
    ...
    public void Total_uses_money_rounding_away_from_zero()
    {
        ...
        // 10.005 => 10.01
        order.AddItem(new OrderItem(Guid.NewGuid(), "Item A", new Money(10.005m, "USD"), 1));
    ...
    public void RemoveItem_removes_item_and_recalculates_total()
    {
        ...
        order.AddItem(new OrderItem(aId, "A", new Money(10m, "USD"), 2)); // 20
        order.AddItem(new OrderItem(bId, "B", new Money(5m, "USD"), 3)); // 15
        Assert.Equal(35m, order.Total.Amount);
    ...
    {
        var order = new CustomerOrder(Guid.NewGuid(), Guid.NewGuid());
        order.AddItem(new OrderItem(Guid.NewGuid(), "A", new Money(10m, "USD"), 1));
    ...
    public void ChangeQuantity_throws_after_confirm()
    {
        ...
        var aId = Guid.NewGuid();
        order.AddItem(new OrderItem(aId, "A", new Money(10m, "USD"), 1));
        order.Confirm();
    ...
    {
        var order = new CustomerOrder(Guid.NewGuid(), Guid.NewGuid());
        order.AddItem(new OrderItem(Guid.NewGuid(), "Item A", new Money(10m, "USD"), 1));
